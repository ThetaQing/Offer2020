...
private:
	void reset()  // 所有顶点、边的辅助信息复位
	{
		for (int i = 0; i < n; ++i)  // 所有顶点
		{
			...
			parent(i) = -1; priority(i) = INT_MAX;  // 在遍历树中的父节点，优先级数
	...
	void BFS(int, int&);  // 连通域广度优先搜索算法
	void DFS(int, int&);  // 连通域深度优先搜索算法
...
public:
	...
	// 边：这里约定，无向边均统一转化为方向互逆的一对有向边，从而将无向图看作有向图的特例
	int e; //边总数
	virtual bool exists(int, int) = 0; //边(v, u)是否存在
	virtual void insert(Te const&, int, int, int) = 0; //在顶点v和u之间揑入权重为w的边e
